module.controller('KotobaFavoritListController', ['$scope', '$http', '$sce','$rootScope','SharedScopes', function($scope, $http, $sce,$rootScope,SharedScopes) {
	var ParamList_R = [];
	var ParamList_W = [];
	ParamList_R = SharedScopes.get();
	SharedScopes.delete();
	var favorit = ParamList_R['favorit'] ;
	//console.log(favorit);
	// ons.ready(function() {
		$http({
			method: 'POST',
			url: host + '/php/select/kotoba_favorit_list.php', 
			headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
			data: $.param({
				session_id: localStorage.getItem('session_id'),
				user_id: id,
				item_note_id: favorit.id
			})
		})
